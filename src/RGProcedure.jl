...
function rg_procedure(velocity::Array{Float64,2}, dielectric::Array{Float64,2}, velocity_integrand::Function, dielectric_integrand::Function, m::Int64, n::Int64)
    ...
    for i in 2:m
        ...
        for j in 1:n
            ...
            velocity[j, m-i+1] = velocity[j, m-i+2] + dcutoff * quadgk(velocity_integrand_phi, 0.0, pi / 2.0, rtol=1e-4)[1]
            dielectric[j, m-i+1] = dielectric[j, m-i+2] + dcutoff * quadgk(dielectric_integrand_phi, 0.0, pi / 2.0, rtol=1e-4)[1]
        end
    end
end
...
function rg_procedure(velocity::Array{Float64,1}, dielectric::Array{Float64,1}, velocity_integrand::Function, dielectric_integrand::Function, m::Int64, n::Int64)
    ...
    for i in 2:m
        ...
        for j in 1:n
            ...
            dielectric_integrand_phi(phi) = dielectric_integrand(velocity, dielectric, momentum, cutoff, phi, m, n)
            ## Solving ODE's using Euler method
            new_velocity[j] = velocity[j] + dcutoff * quadgk(velocity_integrand_phi, 0.0, pi / 2.0, rtol=1e-4)[1]
            new_dielectric[j] = dielectric[j] + dcutoff * quadgk(dielectric_integrand_phi, 0.0, pi / 2.0, rtol=1e-4)[1]
        end
...
function rg_procedure(velocity::Function, dielectric::Function, velocity_integrand::Function, dielectric_integrand::Function, m::Int64, n::Int64=100)
    ...
    for i in 2:m
        ...
        for j in 1:n
            ...
            ## Solving ODE's using Euler method
            new_velocity[j] = velocity(momentum) + dcutoff * quadgk(velocity_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
            new_dielectric[j] = dielectric(momentum) + dcutoff * quadgk(dielectric_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
        end
    ...
    for i in 2:m
        ...
        for j in 1:n
            ...
            for k in 1:length(functions)
                ...
                functions[k][j, m-i+1] = functions[k][j, m-i+2] + dcutoff * quadgk(integrand_phi, 0.0, pi / 2.0, rtol=1e-4)[1]
            end
        end
    end
end
...
function rg_procedure(functions::Array{Array{Float64,1},1}, functions_integrand::Array{Function,1}, m::Int64, n::Int64)
    ...
    for i in 2:m
        ...
        for j in 1:n
            ...
            for k in 1:length(functions)
                integrand_phi(phi) = functions_integrand[k](functions, momentum, cutoff, phi, m, n)
                new_functions[k, j] = functions[k][j] + dcutoff * quadgk(integrand_phi, 0.0, pi / 2.0, rtol=1e-4)[1]
            end
        end
...
function rg_procedure_rk2(velocity::Function, dielectric::Function, velocity_integrand::Function, dielectric_integrand::Function, m::Int64, n::Int64=100)
    ...
    for i in 2:m
        ...
        for j in 1:n
            ...
            ## Solving ODE's using Euler method
            new_velocity[j] = velocity(momentum) + 1 / 2 * dcutoff * quadgk(velocity_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
            new_dielectric[j] = dielectric(momentum) + 1 / 2 * dcutoff * quadgk(dielectric_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
        end
...
function rg_procedure_rk4(velocity::Function, dielectric::Function, velocity_integrand::Function, dielectric_integrand::Function, m::Int64, n::Int64=100)
    ...
    for i in 2:m
        ...
        for j in 1:n
            ...
            ## Solving ODE's using Euler method
            temp_velocity[j] = velocity(momentum) + 1 / 2 * dcutoff * quadgk(velocity_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
            temp_dielectric[j] = dielectric(momentum) + 1 / 2 * dcutoff * quadgk(dielectric_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
        end
        ...
        for j in 1:n
            ...
            ## Solving ODE's using Euler method
            temp_velocity[j] = velocity(momentum) + dcutoff * quadgk(velocity_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
            temp_dielectric[j] = dielectric(momentum) + dcutoff * quadgk(dielectric_integrand_phi, 0.0, pi / 2.0, rtol=1e-3)[1]
        end
